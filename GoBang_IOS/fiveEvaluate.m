...
#import "fiveEvaluate.h"

@implementation fiveEvaluate
- (id)init{
    [super init];
    ...
    int stateScore[ 8 ] = { 0 , 100000 , 10000 , 5000 , 3000 , 500 , 100 , 50 } ;//对每一种状态所对应的分数
    memset( numOfState , 0 , sizeof( numOfState ) ) ;//初始化为0
}
    
    
//对单独的一行进行分析,line表示一行的棋子情况
void analyseLine( int player , NSMutableString * line ) //对某一行(水平，垂直或倾斜)进行分析
{
    int enemy = ( player == MYSELF ) ? RIVAL : MYSELF ;//对手
    int current_player = player ;//将字符转换成int，以利于计算当前选手各种状态的个数
...
//寻找各个状态的个数
void Evaluate::findStateNum( int board[ MAX_LINE ][ MAX_LINE ] )
{
    //在水平和垂直方向搜索
    ...
    //左上-右下搜索
    for( int sub = 10 ; sub >= - 10 ; -- sub )
    {
        string src = "" ;
        for( int i = ( sub >= 0 ) ? sub : 0 , j = i - sub ; i <= 14 && j <= 14 ; ++ i , ++ j )
        {
            src += board[ i ][ j ] ;
        }
        analyseLine( MYSELF , src ) ;
...
//注:现在计算的是我方刚走完后的棋局面
int Evaluate::getValue( int player , int board[ MAX_LINE ][ MAX_LINE ] , int *whether_win )
{
